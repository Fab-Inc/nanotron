...
if __name__ == "__main__":
    ...
    if isinstance(LOG_BASE_DIR, Path):
        logging_dir = LOG_BASE_DIR / savefold
        logging_dir.mkdir(exist_ok=True, parents=True)
    else:
        logging_dir = f"{LOG_BASE_DIR}/{savefold}"
    ...
    for config in configs:
        ...
        local_working_dir = str(LOCAL_WORKING_DIR / config)
        prev = LocalPipelineExecutor(
            pipeline=[
                ...
                #     save_filename=f"{DATASET_NAME}",  # _{config}",
    ...
    #         )
    #     ],
    #     logging_dir=str(LOG_BASE_DIR / "merger"),
    #     depends=prev,
    # )
