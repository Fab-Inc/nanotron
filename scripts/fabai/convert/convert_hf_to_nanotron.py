...
import nanotron
from nanotron.config import Config, GeneralArgs, ModelArgs, TokenizerArgs
...
def convert_hf_to_nt(
    model_hf: LlamaForCausalLM,
    model_nt: LlamaForTraining,
    ...
    for module_name_nt, module_nt in model_nt.named_modules():
        for param_name_nt, param_nt in module_nt.named_parameters(recurse=False):
            ...
            if "qkv_proj" in module_name_nt:
                key_k, key_q, key_v = sorted(
                    nt_to_hf[f"{module_name_nt}.{param_name_nt}"]
                )
                q = hf_sd[key_q]
                k = hf_sd[key_k]
                v = hf_sd[key_v]
...
def convert_checkpoint_and_save(checkpoint_path: Path, save_path: Path):
    ...
    model_args = ModelArgs(
        model_config=model_config, init_method=RandomInit(std=0.2), dtype="bfloat16"
    )
    nanotron_config = Config(
        general=GeneralArgs(project="azure-test"),
        model=model_args,
        parallelism=parallel_config,
        tokenizer=TokenizerArgs(str(checkpoint_path))
    )
    convert_hf_to_nt(hf_model, nanotron_model, model_config)
    nanotron.serialize.save_weights(
        model=nanotron_model, parallel_context=parallel_context, root_folder=save_path
    )
    with open(save_path / "model_config.json", "w+") as f:
        json.dump(dataclasses.asdict(model_config), f)
    with open(save_path / "config.yaml", "w+") as f:
        yaml.safe_dump(
            dataclasses.asdict(nanotron_config, dict_factory=_yaml_dict_factory), f
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        default="llama-7b-hf",
        help="Path to save the nanotron model",
    )
