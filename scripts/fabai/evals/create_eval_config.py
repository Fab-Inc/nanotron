...
def create_lighteval_config(
    ...
    logging_args = LightEvalLoggingArgs(local_output_path=output_dir)
    ...
    # slurm config
    slurm_config = LightEvalSlurm(gpus_per_node=1, partition="debug", cpus_per_task=18)
...
def save_lighteval_config_as_yaml(config: LightEvalConfig, output_path: str) -> None:
    def dict_factory(x):
        ...
        for key, val in dict(x).items():
            ...
            elif isinstance(val, SamplerType):
                out[key] = val.name.lower()
            elif isinstance(val, (str, int, float, tuple, list, dict)):
                out[key] = val
        return out
    ...
    with open(output_path, "w") as f:
        ...
        # config_dict = {
        #     "logging": {
        #         "output_dir": str(config.logging.local_output_path.resolve()),
        #         "push_to_hub": config.logging.push_results_to_hub,
        #         "push_to_tensorboard": config.logging.push_results_to_tensorboard,
